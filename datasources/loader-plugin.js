(function() {
	...
	var loaderDatasource = function(settings, updateCallback) {
      ...
      console.log(event);
			...
			if (event.name === 'change-dashboard') {
				//fetch dashboard data and load it
	      $.get('/' + event.value + '.json', function (data) {
	...
	freeboard.loadDatasourcePlugin({
			type_name: "loaderDatasource",
			display_name: "Change Dashboard",
			description: "Loads a new dashboard when a socket.io message comes in from the server.",
			...
			newInstance : function(settings, newInstanceCallback, updateCallback) {
				newInstanceCallback(new loaderDatasource(settings, updateCallback));
			}
		});
}());
